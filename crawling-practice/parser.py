...
def crawler_thesis_chk_setting():
    ...
    :return:
    """
    path = "C:/Users/multicampus/Downloads/chromedriver"
    ...
    login.send_keys("hinu93")
    login = driver.find_element_by_name("upw")
    login.send_keys("okey102938!")
    login.send_keys('\n')
    ...
    for i in range(3, 4):  # range(1, int_last_page) 추후에 모든 페이지 excel로 출력
        driver.execute_script("javascript:goPage(" + str(i) + ")")
        driver.find_element_by_id("checkAll").click()
        driver.execute_script("lf_exceldown()")

        while not os.path.exists(download_path + "논문검색리스트Excel (" + str(i) + ").xls"):
            time.sleep(1)
